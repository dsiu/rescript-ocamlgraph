...
module type Data = {
  ...
  let analyze: (edge, t) => t

  let widening: (t, t) => t
}

module Make = (G: G, D: Data with type edge = G.E.t) => {
  ...
  let recurse = (g, wto, init, widening_set, widening_delay) => {
    ...
    let find = (vertex, data) =>
      try M.find(vertex, data) catch {
      | Not_found => init(vertex)
      }

    let analyze_vertex = (widening_steps, do_widen, v, data) => {
      /* Computes the result of the analysis for one vertex */
      let result = G.fold_pred_e((edge, acc) => {
        let src = G.E.src(edge)
        let data_src = find(src, data)
        let data_dst = D.analyze(edge, data_src)
        D.join(data_dst, acc)
      }, g, v, init(v))
      if widening_steps <= 0 && do_widen(v) {
        D.widening(find(v, data), result)
      } else {
        result
      }
    }
    ...
    and stabilize = (can_stop, widening_steps, head, comps, data) => {
      /* Iterates the analysis of a component until
         ...
         stabilisation. */
      let old_data_head = find(head, data)
      let new_data_head = analyze_vertex(widening_steps, do_head_widen, head, data)
    ...
    and analyze_element = (widening_steps, data, x) =>
