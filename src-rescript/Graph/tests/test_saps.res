...
let g3 = List.fold_left(
  (g, (src, dst)) => GS.add_edge(g, src, dst),
  GS.empty,
  list{
    ("A", "B"),
    ("B", "A"),
...
let pp_set = (pp_ele, pf, s) => {
  open Format
  fprintf(pf, "@[<hv 4>{%a}@]", pp_print_list(~pp_sep=pp_comma, pp_ele), s)
}

let () = {
  let saps1 = CI.sstrong_articulation_points(g1)
  let saps2 = CI.sstrong_articulation_points(g2)
  let saps3 = CS.sstrong_articulation_points(g3)
  let saps4 = CI.sstrong_articulation_points(g4)
  let saps5 = CI.sstrong_articulation_points(g5)
  open Format
  printf(
    "@[<v>%a@,%a@,%a@,%a@,%a@]@.",
    pp_set(pp_print_int),
